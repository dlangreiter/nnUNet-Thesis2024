...
def display_masks_slice_combined_with_anatomical(anatomical_image, gt_mask, pred_mask, overlap_mask, slice_idx, cmap_gt, cmap_pred, cmap_overlap, ax):
    ...
    - gt_mask: 3D numpy array, ground truth mask
    - pred_mask: 3D numpy array, prediction mask
    ...
    print(f"Slice {slice_idx}: Prediction unique values: {np.unique(pred_slice)}")
...
def display_masks_grid_with_anatomical(anatomical_image, ground_truth, prediction, cmap_gt, cmap_pred, cmap_overlap, slice_indices, grid_size=(3,3), save_path=None):
    ...
    - prediction: 3D numpy array, prediction mask
    - cmap_gt: matplotlib Colormap object for Ground Truth
    - cmap_pred: matplotlib Colormap object for Prediction
    - cmap_overlap: matplotlib Colormap object for Overlap
    ...
    for idx, slice_idx in enumerate(slice_indices):
        ...
        gt_overlay, pred_overlay, overlap_overlay = display_masks_slice_combined_with_anatomical(
            ...
            ground_truth,
            prediction,
            ground_truth & prediction,  # Overlap
            slice_idx,
            cmap_gt,
            cmap_pred,
            cmap_overlap,
    ...
    black_patch = mpatches.Patch(color='black', label='Overlap')
    background_patch = mpatches.Patch(color='gray', label='Background')
    # Position the legend outside the grid
    fig.legend(handles=[red_patch, yellow_patch, black_patch, background_patch],
...
def create_interactive_slideshow(anatomical_image, ground_truth, prediction, cmap_gt, cmap_pred, cmap_overlap, interval=500, save=False, save_path='slideshow.gif'):
    ...
    # Calculate Overlap
    overlap = ground_truth & prediction  # Overlap where both masks are present
...
def main():
    ...
    anatomical_image_path = '/home/declan/thesis7/10percenttest/input/BRAT_263_0000.nii.gz'  # Replace with your anatomical NIfTI file path
    ground_truth_path = '/home/declan/thesis7/10percenttest/ground_truth/BRAT_263.nii.gz'      # Replace with your ground truth NIfTI file path
    prediction_path = '/home/declan/thesis7/resultPostprocess/BRAT_263.nii.gz'                # Replace with your prediction NIfTI file path
    ...
    cmap_pred = ListedColormap(['#00000000', '#FFFF00FF'])    # Prediction: Transparent and Yellow
    cmap_overlap = ListedColormap(['#00000000', '#000000FF']) # Overlap: Transparent and Black
